...
map.on('click', async function(e){
    const url = getFeatureInfoUrl(map, wmsLayers["Quartiers"], e.latlng,{info_format:'application/json', feature_count:1});
    try{
        ...
        clickLayer = L.geoJSON(f.geometry,{style:{color:'#28a745', weight:3, fillOpacity:0.2}}).addTo(map);
        map.fitBounds(clickLayer.getBounds(),{padding:[40,40], maxZoom:18, animate:true});
        L.popup().setLatLng(clickLayer.getBounds().getCenter())
                 .setContent(`<b>Quartier :</b> ${f.properties.quartiers||'Nom inconnu'}`)
                 .openOn(map);
